...
import ProductCategoryFilter from "./ProductCategoryFilter"; // Import the filter component

export default function ProductByCategory() {
  const { getProductsByCategoryId } = useContext(ProductContext);
  const secretKey = import.meta.env.VITE_SECRET_KEY;
  const [decryptedURLId, setDecryptedURLId] = useState("");
  const [allCategoryProducts, setAllCategoryProducts] = useState([]); // Store all products
  ...
  }, [selectedCategories, priceRange, selectedRatings, allCategoryProducts]);
    ...
    <div className="bg-[#F9F7FC]">
      <div className="py-4 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 bg-[#F9F7FC]">
        ...
        <div className="flex flex-col md:flex-row gap-6">
          {/* Filters Sidebar - Only show if we have products */}
          ...
          {/* Products Listing */}
          <div className={`${allCategoryProducts.length > 0 ? 'w-full md:w-3/4' : 'w-full'}`}>
